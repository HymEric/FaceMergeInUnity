...
public class Youtu
    ...
    public static string multifaceidentify(string image_path, string group_id, List<string> group_ids, int topn = 5, int min_size = 40)
    {
        ...
        pars = string.Format(pars, Conf.Instance().APPID, group_id, LitJson.JsonMapper.ToJson(group_ids.ToArray()), Utility.ImgBase64(image_path), topn, min_size);
        postData.Append("{");
        postData.Append(pars);
        postData.Append("}");
        string result = Http.HttpPost(methodName, postData.ToString(), Auth.appSign(expired, Conf.Instance().USER_ID));
    ...
    public static string multifaceidentifyurl(string url, string group_id, List<string> group_ids, int topn = 5, int min_size = 40)
    {
        ...
        pars = string.Format(pars, Conf.Instance().APPID, group_id, LitJson.JsonMapper.ToJson(group_ids.ToArray()), url, topn, min_size);
        postData.Append("{");
        postData.Append(pars);
        postData.Append("}");
    ...
    #region ************************************************************************************************************* 图像处理
    /// <summary>
    /// 人脸融合，根据用户上传的图像，融合后的图片。
    /// </summary>
    /// <param name="image_path">图片路径</param>
    /// <returns>返回的结果，JSON字符串，字段参见API文档</returns>
    public static string faceMerge(string image_path, string rsp_img_type, string opdata)
        {
            string expired = Utility.UnixTime(EXPIRED_SECONDS);
            string methodName = "cgi-bin/pitu_open_access_for_youtu.fcg";
            StringBuilder postData = new StringBuilder();

            string pars = "\"img_data\":\"{0}\",\"rsp_img_type\":\"{1}\",\"opdata\":{2}";
            pars = string.Format(pars, Utility.ImgBase64(image_path), rsp_img_type, opdata);
            postData.Append("{");
            postData.Append(pars);
            postData.Append("}");
            string result = Http.HttpPost(methodName, postData.ToString(), Auth.appSign(expired, Conf.Instance().USER_ID));
    ...
    /// <returns>返回的结果，JSON字符串，字段参见API文档</returns>
    /// 自己的
    public static string faceMergeFrombytes(byte[] image_pathbytes, string rsp_img_type, string opdata)
    {
        string expired = Utility.UnixTime(EXPIRED_SECONDS);
        string methodName = "cgi-bin/pitu_open_access_for_youtu.fcg";
        StringBuilder postData = new StringBuilder();
        ...
        string pars = "\"img_data\":\"{0}\",\"rsp_img_type\":\"{1}\",\"opdata\":{2}";
        pars = string.Format(pars, Utility.MyImgBase64FromBytes(image_pathbytes), rsp_img_type, opdata);
        postData.Append("{");
        postData.Append(pars);
        postData.Append("}");
        string result = Http.HttpPost(methodName, postData.ToString(), Auth.appSign(expired, Conf.Instance().USER_ID));
